...
import type { Student, MonthlyPlan, QuranAchievementReport } from "~/types";
  ...
  try {
    ...
    reports.value = students.value.map((student: Student) => {
      return {
        ...
        month:
          plansStore.monthsPlansData.find(
            (mp) => mp.id === Number(state.monthly_plan_id)
          )?.month || "",
        achieved_pages: 0,
        status: "غير مكتمل",
...
const saveReports = async () => {
  ...
  try {
    ...
    // navigateTo({ name: "quran-achievement-reports" }); // افترض وجود صفحة لعرض التقارير
    students.value = [];
    reports.value = [];
  } finally {
    isLoading.value = false;
...
onMounted(async () => {
  await Promise.all([academicClassesStore.fetchClasses(), getMonthlyPlans()]);
  getRequiredPages();
});
</script>
    ...
    <UCard>
      ...
      <template #default>
        ...
        <div class="flex flex-wrap gap-2 mb-5" v-if="students.length">
          ...
          <div class="p-3 bg-info/15 rounded-md flex gap-3">
