...
const useQuranClassesStore = defineStore("quran_classes", () => {
  ...
  const classesWithStudentsCount = computed(() => {
    return classes.value.map((_class) => {
      const count = studentsStore.sortedStudents.filter(
        (student) => {
          var _a, _b;
          return ((_a = student.quran_class) == null ? void 0 : _a.title) === _class.title && ((_b = student.quran_class) == null ? void 0 : _b.group) === _class.group;
        }
      ).length;
  ...
  const updateQuranClassForStudents = async (studentIds, classId) => {
    try {
      loading.value = true;
...
export { useQuranClassesStore as u };
//# sourceMappingURL=quran_classes-BKWJ4W-E.mjs.map
